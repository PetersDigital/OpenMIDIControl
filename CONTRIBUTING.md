...
flutter test test/midi/pipeline/ump_transport_test.dart
```
Required for:
...
```kotlin
@Test
fun `UMP reconstruction preserves MIDI channel`() {
    // Arrange
    val umpPacket = byteArrayOf(
        0x20.toByte(), // MT=2 (Channel Voice)
    ...
    // Act
    MidiParser.processMidiPayload(umpPacket, 0, 4, System.nanoTime(), false, channel, ...)
