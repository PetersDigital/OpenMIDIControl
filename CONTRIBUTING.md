...
- Telegram captions use `git log -n 10 "${FROM_REF}..HEAD"` (same pattern as `cd_auto_dev.yml`). A "View full changelog →" link to the GitHub compare view is included before the disclaimer. Captions exceeding 1024 chars are automatically truncated.
...
**Telegram changelog**: Beta and RC workflows use `git log -n 10 "${FROM_REF}..HEAD"` for Telegram captions (same pattern as `cd_auto_dev.yml`). Captions exceeding Telegram's 1024-char limit are automatically truncated.

### Telegram Notification Previews

All 15+ CI/CD Telegram message types have interactive HTML/CSS previews in `.github/notify-telegram-HTMLCSS-preview/`.
...
4. Tag the commit on the `main` branch.
5. Tags must follow the `vX.Y.Z` format.
...
2. Push to `beta` or `rc` branch — workflows auto-generate tags like `v0.2.2-beta.1` or `v0.2.2-rc.1`.
3. After stable release, update both fields in `.version` and push to `main`.
