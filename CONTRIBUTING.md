...
- **GitHub Actions** (`/`): Weekly checks (Sunday 04:00 Asia/Kolkata, `ci(actions)` commit prefix) with `security-updates`, `actions-minor-patch`, and `actions-major` groups
- **npm** (`/`): Weekly checks (Sunday 04:00 Asia/Kolkata, `chore(npm)` commit prefix) with a dedicated `security-updates` group
- **Flutter/Pub** (`/app`): Weekly checks (Sunday 04:00 Asia/Kolkata, `chore(deps)` commit prefix) with `security-updates`, `minor-and-patch`, and `major-updates` groups
