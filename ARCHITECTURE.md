...
To enable comprehensive unit testing without requiring Android Service lifecycle mocks, the UMP reconstruction logic was extracted from `MainActivity.kt` into a dedicated, testable static object `MidiParser.kt` (commit `15f8ee8`).
...
- **MidiDeviceService with UMP Transport**: VirtualMidiService and PeripheralMidiService extend Android's `MidiDeviceService` with `TRANSPORT_UNIVERSAL_MIDI_PACKETS` flag for system-wide virtual routing.
