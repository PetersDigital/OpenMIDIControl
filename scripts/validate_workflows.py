...
Automatically fixes common issues like CRLF line endings and trailing spaces.
...
def print_step(step_num: int, text: str) -> None:
    """Print a step header."""
    print(f"{Colors.YELLOW}[{step_num}/4] {text}{Colors.RESET}")
...
def main() -> int:
    ...
    # Step 3: Run actionlint (only on .github/workflows/*.yml files)
    # Note: actionlint validates workflow files only. Cross-validation of
    # composite action inputs requires running actionlint in a CI environment
    # with full repository context (see validate_auto_yaml.yml).
    if workflow_files:
        print_step(3, "Running actionlint (includes shellcheck)...")
    ...
    # Step 4: Run yamllint (on all files)
    print_step(4, "Running yamllint...")
