...
def categorize_commits(commits: List[Tuple[str, str]]) -> Dict[str, List[Tuple[str, str, str]]]:
    ...
    Path-based routing:
    - Commits touching only .github/ → CI/CD Infrastructure
    - Commits touching only scripts/ → Development Tools
    - All other commits → Type-based routing (Added, Fixed, Changed, etc.)

    This keeps infrastructure changes from polluting the main app changelog.
    ...
    for commit_hash, message in commits:
        ...
        # Check which paths this commit touched
        files_changed = _get_files_for_commit(commit_hash)
        has_github = any(f.startswith('.github/') for f in files_changed)
        has_scripts = any(f.startswith('scripts/') for f in files_changed)
        has_other = any(not f.startswith('.github/') and not f.startswith('scripts/') for f in files_changed) if files_changed else True

        # Path-based routing takes priority over type-based
        if has_github and not has_scripts and not has_other:
            sections['CI/CD Infrastructure'].append((commit_hash, '', parsed[1] if parsed else message))
        elif has_scripts and not has_github and not has_other:
            sections['Development Tools'].append((commit_hash, '', parsed[1] if parsed else message))
        elif parsed:
...
"""
    ...
    # Add sections in order (Merged first, then Keep a Changelog standard + Other, then infra at end)
    for section_name in ['Merged', 'Added', 'Changed', 'Deprecated', 'Removed', 'Fixed', 'Security', 'Other', 'CI/CD Infrastructure', 'Development Tools']:
