...
- Git log retrieval
- Tag detection
- Path-based changelog routing (CI/CD vs Development Tools vs App)
...
def test_release_notes_generation(tag_info: Optional[Tuple[str, int, str]]) -> None:
    ...
    if tag_info:
        ...
        print("-" * 47)
        print("[BETA] **Beta Release** - This is a release candidate for testing.\n")
...
def main() -> int:
    ...
    test_path_based_changelog_routing()
...
def test_path_based_changelog_routing() -> None:
    """Test path-based changelog routing logic."""
    print_section("Test 9: Path-Based Changelog Routing")
    print("Commits touching only .github/ -> CI/CD Infrastructure")
    print("Commits touching only scripts/ -> Development Tools")
    print("All other commits -> Type-based routing\n")

    # Test case: pure .github/ changes
    test_cases = [
        ([".github/workflows/cd_auto_beta.yml"], False, False, True, "CI/CD Infrastructure"),
        ([".github/actions/generate-release-notes/action.yml"], False, False, True, "CI/CD Infrastructure"),
        (["scripts/add_license_headers.py"], True, False, False, "Development Tools"),
        (["scripts/validate_workflows.py"], True, False, False, "Development Tools"),
        (["app/lib/ui/open_midi_screen.dart"], False, True, False, "Type-based"),
        (["README.md"], False, True, False, "Type-based"),
        ([".github/workflows/cd_auto_beta.yml", "app/lib/ui/midi_service.dart"], False, True, True, "Type-based"),  # Mixed
        (["scripts/commit.py", "README.md"], True, True, False, "Type-based"),  # Mixed
    ]

    for files, has_scripts, has_app, has_github, expected in test_cases:
        # Replicate bash routing logic
        is_pure_github = has_github and not has_scripts and not has_app
        is_pure_scripts = has_scripts and not has_github and not has_app

        if is_pure_github:
            result = "CI/CD Infrastructure"
        elif is_pure_scripts:
            result = "Development Tools"
        else:
            result = "Type-based"

        if result == expected:
            print_ok(f"{files} -> {result}")
        else:
            print_fail(f"{files} -> {result} (expected: {expected})")
