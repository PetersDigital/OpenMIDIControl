...
def get_diff_stat() -> str:
    """Get git diff stat for auto-body generation."""
    success, stat = run_command(['git', 'diff', '--cached', '--stat'])
    if success and stat.strip():
        return stat.strip()
    return ""
...
def interactive_mode(no_verify: bool = False) -> int:
    ...
    # Step 2: Select scope (with suggestion)
    scopes = get_user_scopes()
    scope = select_from_list(scopes, "Scope (Optional)")
    ...
    print(f"  {Colors.YELLOW}•{Colors.RESET} Auto-generated: git diff --stat summary")
    print(f"  {Colors.YELLOW}•{Colors.RESET} BREAKING CHANGE: Major API changes")

    body_choice = input(f"\n{Colors.CYAN}Add body? (a=auto, y=yes, N=no): {Colors.RESET}").strip().lower()
    ...
    while True:
        ...
        print(f"{Colors.CYAN}2. Add/edit body{Colors.RESET}")
        print(f"{Colors.CYAN}3. Start over{Colors.RESET}")
