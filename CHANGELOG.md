...
- **Automated Dependency Updates**: Dependabot configuration now runs on a weekly cadence for GitHub Actions (`/`), npm (`/`), and Flutter pub dependencies (`/app`), with dedicated `security-updates` groups (`applies-to: security-updates`) plus a pub open-PR limit of 5.
- **YAML Linting & Validation Scope**: Integrated yamllint with custom rules (document-start, line-length 150, trailing-spaces, unix newlines) and actionlint for schema validation with Dependabot PR exemptions. Validation scope excludes docs-only changes and `.github/notify-telegram-HTMLCSS-preview/**` from YAML/license workflows.
...
- **Commit Message Validation**: Added commitlint workflow (`validate_pr_commitlint.yml`) and Husky pre-commit hook enforcing Conventional Commits format on all branches, with cancel-in-progress PR concurrency to avoid redundant runs.
