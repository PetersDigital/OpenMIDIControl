...
7. `notify-telegram` - Sends beta APK directly via `curl -F document=@...` with changelog caption (**waits for provenance**)
...
4. `cosign-sign-verify` - Sign all artifacts with keyless OIDC
5. `provenance` - SLSA provenance attestation — conditional on build success
6. `create-release-candidate` - **Public** prerelease with generated notes — conditional on provenance success (**not a draft**)
7. `notify-telegram` - Sends RC APK directly via `curl -F document=@...` with changelog caption (**waits for provenance**)

**Artifacts:** Signed APK + ZIP + Provenance (sent to Telegram + public prerelease)
