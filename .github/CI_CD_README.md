...
- Built-in retries, timeout, and API success validation
...
**Notification:** RC release enterprise notification with tiered layout and action buttons
...
**Telegram Behavior:** Uses the `notify-telegram` composite action (`notification-type: rc-build`) with standardized enterprise sections (`🧭 Actions`, `📊 Signals`) and a direct release link.
...
- All notification types route through `notify-telegram` composite action with standardized enterprise templates
- Every message includes linked Repository, Branch/Tag, Commit, and Workflow URLs
- Beta and RC notifications include direct release links with standardized enterprise section labels
- Dev notifications can deliver artifacts through the same composite action (no ad-hoc Telegram curl paths)
- Action supports inline buttons, delivery retries, and structured diagnostics
- Runtime notifications and HTML previews share the same canonical vocabulary and emoji map (`🧭 Actions`, `🛠 Diagnostics`, `📊 Signals`, `🛡 Trust Signals`)
- Workflows apply strict `notify-status` + regex gates (`notify-types-regex`, `notify-branches-regex`, `notify-tags-regex`) to reduce noise in a single Telegram chat

**Changes Made:**
- **`notify-telegram/action.yml`:** Standardized tiered enterprise message structure, canonical section labels, and emoji usage across success/failure/skipped templates
...
- **Preview synchronization:** Updated all Telegram preview pages to mirror runtime message labels, separators, and emoji markers

**Result:**
- ✅ Dev builds send APK files through the same enterprise composite action
- ✅ Beta/RC/prod/hotfix/manual flows share one structured notification system
- ✅ Success and failure density are context-aware (compact success, richer failure diagnostics)
- ✅ Runtime notifications and previews now use one canonical vocabulary/emoji profile
- ✅ Delivery reliability improved with retries, timeout, and API-level success checks
- ✅ Long messages are truncated safely to avoid Telegram size failures
