...
**Used by:** Release publishing workflows to collect Android and Windows builds. Also downloads `release_notes.md` artifact in beta/RC workflows so `notify-telegram` can extract changelog for Telegram captions.
...
- `release_reason`: Reason for release (from check-release-markers)
- `skip_reason`: Reason for skipping release (from check-release-markers)
...
**Design Decision:** This action sends **text-only notifications** using simple `curl` commands. All notification types (dev, beta, RC, release, hotfix, CI failures) route through this composite action for consistent message formatting. This avoids HTTP 000 errors from complex multipart upload logic.

**Features:**
- HTML parse mode with user-input escaping (prevents injection)
- Standardized template across all notification types with linked Repository, Branch/Tag, Commit, and Workflow URLs
- Direct release links on beta and RC notifications with issue-reporting footer
...
|---|------|----------|--------|---------|
| 1 | Dev Build | `cd_auto_dev.yml` | Text message | Repository, branch, commit, changelog, workflow link |
| 2 | Beta Build | `cd_auto_beta.yml` | Text message | Beta tag, branch, commit, release link, workflow link |
| 3 | RC Build | `cd_auto_rc.yml` | Text message | RC tag, branch, commit, release link, workflow link |
| 4 | Hotfix Release | `cd_man_hotfix.yml` | Text message | Base version, patch number, commit, release link, workflow link |
| 5 | Prod Release | `cd_auto_prod.yml` | Text message | Tag, commit, release link, workflow link, provenance status |
| 6 | Manual Prod Rebuild | `cd_man_prod.yml` | Text message | Tag, commit, release link, workflow link, provenance status |
| 7 | Manual Legacy Rebuild | `cd_man_retro.yml` | Text message | Tag, commit, release link, workflow link, provenance status |
| 8 | Build Failure | `cd_man_prod.yml`, `cd_man_retro.yml` | Text message (release failure) | Tag, commit, workflow link |
| 9 | CI Pipeline Failed | all CI/CD workflows | Text message | Branch, commit, actor, workflow link |
| 10 | Build Skipped | `cd_auto_rc.yml` | Text message | Branch, actor, reason, workflow link |
| 11 | Dev Build Failed | `cd_auto_dev.yml` | Text message | Branch, commit, actor, workflow link |
| 12 | Beta Build Failed | `cd_auto_beta.yml` | Text message | Beta tag, branch, commit, actor, workflow link |
| 13 | RC Build Failed | `cd_auto_rc.yml` | Text message | RC tag, branch, commit, actor, workflow link |
| 14 | Hotfix Failed | `cd_man_hotfix.yml` | Text message | Base version, patch number, actor, workflow link |
| 15 | Release Failed | `cd_auto_prod.yml`, `cd_man_prod.yml`, `cd_man_retro.yml` | Text message | Tag, actor, workflow link |

All messages use a **standard template** with linked Repository, Branch/Tag, Commit, and Workflow URLs. Beta and RC notifications include a direct release link and an issue-reporting footer pointing to the repository's `/issues` page.

### HTML/CSS Telegram Preview

All 10+ Telegram message types have interactive HTML/CSS previews in `.github/notify-telegram-HTMLCSS-preview/`. These render pixel-accurate Telegram phone frame mockups for documentation and review purposes.
...
├── telegram-01-dev-build.html     # Dev Build (single APK caption)
├── telegram-02-beta-build.html    # Beta Build (single APK caption)
├── telegram-03-rc-build.html      # RC Build (single APK caption)
├── telegram-04-hotfix.html        # Hotfix Release (text)
├── telegram-05-prod-release.html  # Production Release (text)
├── telegram-06-manual-prod.html   # Manual Prod Rebuild — Success
├── telegram-07-manual-retro.html  # Manual Legacy Rebuild — Success
├── telegram-08-build-failure.html # Build Failure (custom-message)
├── telegram-09-ci-failed.html     # CI Pipeline Failed (generic)
...
**Artifacts:** Signed APK + ZIP + Provenance (published to draft release)
**Notification:** Beta release text notification with tag, branch, commit, and actor
...
**Telegram Behavior:** Uses the `notify-telegram` composite action (`notification-type: beta-build`) with standardized text message: beta tag, branch, commit, release link, workflow link, and an issue-reporting footer.
**Release Type:** Draft + prerelease (hidden until manually published).
...
**Jobs:**
1. `analyze-and-test` - Flutter CI core — **Always runs** on push (skips for Dependabot)
2. `build-rc` - Reads `pubspec.yaml` version; if not yet a stable GitHub release, uses it. Otherwise falls back to `NEXT_PLANNED_VERSION` from `.version`. Generates sequential RC tag, builds Android APK
...
7. `notify-telegram` - Sends RC release notification via `notify-telegram` composite action

**Artifacts:** Signed APK + ZIP + Provenance (published to public prerelease)
**Notification:** RC release text notification with tag, branch, commit, and actor

**Version Tracking:** Same logic as beta — `pubspec.yaml` first, fallback to `NEXT_PLANNED_VERSION` from `.version` when pubspec version is already a stable release. Tag generation: `v{version}-rc.1`, `v{version}-rc.2`, etc. See [Version Tracking](#version-tracking) below.

**Telegram Behavior:** Uses the `notify-telegram` composite action (`notification-type: rc-build`) with standardized text message: RC tag, branch, commit, release link, workflow link, and an issue-reporting footer.
...
#### `ci_auto_feature.yml`
**Trigger:** Push to all branches except `main`, `dev`, `beta`, `rc`
**Path filters:** `app/**`, `.github/**`, `scripts/**`, `pubspec.yaml` (excludes `*.md`, `docs/**`)
...
|-------|---------|---------|
| `CURRENT_STABLE_RELEASE_VERSION` | Latest stable release available to users | `from-ref` for `git log` changelog in Telegram captions |
...
4. Beta/RC notification jobs call `notify-telegram` composite action for text-only notifications.
...
**Important workflow behavior:** The workflow checks `gh release view v{version}` to determine if the pubspec version is already stable. This means you can safely bump both files to `0.2.3` before `v0.2.2` is tagged — the workflow will still target `v0.2.2-rc.1` until `v0.2.2` is actually released.
...
**New Files:**
- All 11 composite actions in `.github/actions/`
...
- Beta/RC Telegram captions showed "No changes since last release" — `release_notes.md` was generated in `create-draft-release` but never passed to `notify-telegram`
- Telegram "caption is too long" errors when full changelog exceeded 1024-char limit
- Bash syntax errors from unescaped quotes in changelog content injected into `printf` format strings
- `notify-telegram` action not found error — `notify-telegram` job in beta/RC workflows was missing `actions/checkout` step
- Release notes heading showed "All Changes since v0.2.2" instead of actual previous stable release (v0.2.1)

**Current Architecture:**
- All notification types route through `notify-telegram` composite action with standardized text templates
- Every message includes linked Repository, Branch/Tag, Commit, and Workflow URLs
- Beta and RC notifications include direct release links and issue-reporting footers
- Changelog content no longer embedded in Telegram messages — full notes available on GitHub Releases

**Changes Made:**
- **`notify-telegram/action.yml`:** All message templates standardized with linked URLs; added `Commit` to release/hotfix/dev failure templates; added `By:` to dev-build failure
- **`generate-release-notes/action.yml`:** Fixed RC.1 and hotfix patch.1 headings to use `PREV_TAG` instead of `BASE_VERSION`
- **`cd_auto_beta.yml`** / **`cd_auto_rc.yml`:** Added `actions/checkout` step to `notify-telegram` job
- **`validate_workflows.py`:** Removed emoji-stripping step; UTF-8 natively supports emojis
- **`generate-prerelease-tag/action.yml`:** centralizes beta/RC tag generation and removes fragile `grep` + shell `-e` failure paths.
- **Version tracking:** Tag generation reads `pubspec.yaml` first; only falls back to `.version` when pubspec version is already a stable release. Uses `working-directory: '.'` override for repo root access.
- **Job rename:** `build-rc` → `build-beta` in `cd_auto_beta.yml` for clarity

**Result:**
- ✅ Dev builds send APK files to Telegram with commit changelogs
- ✅ Beta builds send reliable Telegram push notifications via composite action
- ✅ RC builds send reliable Telegram push notifications via composite action
...
- ✅ No more "caption too long" errors — automatic truncation
- ✅ No more bash syntax errors from special characters — `--data-urlencode` used
