...
The project uses three streamlined scripts for license header management:

### Add Headers (All File Types)

To add license headers to all source files:
...
This script:
- Scans all `.dart`, `.kt`, `.ps1`, and `.yml` files
- Skips generated files (`.g.dart`, `.freezed.dart`)
- Skips build directories and third-party configs
- Adds headers only to files missing them
- Preserves UTF-8 encoding without BOM
- Handles shebang lines correctly

### Check Headers (Windows - Local Development)

To verify all files have proper license headers on Windows:

```powershell
.\scripts\check_license_headers.ps1
```

Returns:
- Exit code `0` if all files have headers
- Exit code `1` if any files are missing headers

### Check Headers (Linux/macOS - CI/CD)

To verify all files have proper license headers on Linux/macOS:

```bash
./scripts/check_license_headers.sh
```

Used by GitHub Actions CI. Returns:
- Exit code `0` if all files have headers
- Exit code `1` if any files are missing headers
...
The copyright year is currently `2026`. To update:

1. Update the add script:
   ```powershell
   (Get-Content .\scripts\add_all_license_headers.ps1) -replace '2026', '2027' |
     Set-Content .\scripts\add_all_license_headers.ps1
   ```

2. Update the check scripts:
   ```powershell
   (Get-Content .\scripts\check_license_headers.ps1) -replace '2026', '2027' |
     Set-Content .\scripts\check_license_headers.ps1
   
   (Get-Content .\scripts\check_license_headers.sh) -replace '2026', '2027' |
     Set-Content .\scripts\check_license_headers.sh
   ```

3. Re-run headers to update existing files:
   ```powershell
   .\scripts\add_all_license_headers.ps1
   ```
